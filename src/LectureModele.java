import java.io.BufferedReader;
import java.io.File;
import java.io.FileNotFoundException;
import java.io.FileReader;
import java.io.IOException;

public class LectureModele {
	
	private Point[] points;
	private Face[] faces;

	public LectureModele(){
		String ligne="";
		File inputFile ;
		FileReader myReader;
		BufferedReader myBufferedReader;
		try {
			inputFile = new File("TextFileFolder/Modele.txt");
			myReader= new FileReader(inputFile);
			myBufferedReader = new BufferedReader(myReader);
			try {
				while(!(ligne= myBufferedReader.readLine()).equals("end_header")){
					String[] parts = ligne.split(" ");
					if(parts[0].equals("element") && parts[1].equals("vertex")){
						points=new Point[Integer.parseInt(parts[2])];
					}
					
					if(parts[0].equals("element") && parts[1].equals("face")){
						faces=new Face[Integer.parseInt(parts[2])];
					}
				}
				
				for(int i=0; i<points.length; i++) {
					ligne=myBufferedReader.readLine();
					String[] parts=ligne.split(" ");
					points[i]=new Point(Float.parseFloat(parts[0]),Float.parseFloat(parts[1]),Float.parseFloat(parts[2]));
				}
				
			} catch (IOException e) {
				e.printStackTrace();
			}
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
	}
	
	
	
	
}
