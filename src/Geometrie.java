...
abstract class Geometrie {
	...
	static MatriceFloat rotationZ(double angle) {
		float[][] m=new float[4][4];
		...
		m[2][0]=0; m[2][1]=0; m[2][2]=1; m[3][2]=0;
		m[3][0]=(float)(Main.milieu.getX()*(-Math.cos(angle)+1)+Main.milieu.getY()*Math.sin(angle)); 
		m[3][1]=(float)(Main.milieu.getX()*(-Math.cos(angle)+1)-Main.milieu.getX()*Math.sin(angle)); 
		m[3][2]=0; m[3][3]=1;
		return new MatriceFloat(m);
	}
